...
class DosScanner internal constructor(
    ...
    override fun scanPackage(nestedProvenance: NestedProvenance?, context: ScanContext): ScanResult {
        ...
        val scanResults = runBlocking {
            ...
            val dosPackages = context.coveredPackages.getDosPackages(provenance)

            logger.info { "Packages requested for scanning: ${dosPackages.joinToString { it.purl }}" }
            ...
            when (existingScanResults?.state?.status) {
                "no-results" -> {
                    ...
                    runCatching {
                        ...
                        runBackendScan(dosPackages, sourceDir, startTime, issues)
                    }.onFailure {
                        issues += createAndLogIssue(name, it.collectMessages())
                    }.getOrNull()
                }
    ...
    internal suspend fun runBackendScan(
        dosPackages: List<PackageInfo>,
        sourceDir: File,
        startTime: Instant,
        issues: MutableList<Issue>
    ): ScanResultsResponseBody? {
        logger.info { "Initiating a backend scan for ${dosPackages.map { it.purl } }." }
        ...
        val jobResponse = client.addScanJob(zipName, dosPackages)
        val id = jobResponse?.scannerJobId

        if (id == null) {
            issues += createAndLogIssue(name, "Failed to add scan job for '$zipName' and $dosPackages.")
            return null
        }
